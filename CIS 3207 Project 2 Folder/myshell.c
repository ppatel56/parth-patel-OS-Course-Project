...
int redirection (char *input) { 
    ...
    FILE *inputFile;
    FILE *outputFileAppend;
    FILE *outputFileTrunct;
    int fileDes0, fileDes1; //fileDescript0 == "<" and fileDes1 == ">" or ">>"
    char appendFileOutputName[30];
    char fileOutputName[30]; //Don't think making it a pointer is necessary
    char fileInputName[30];
   ...
   if(getCommandInput(input, args)==0){
    ...
    while(args[index] != NULL) { 
        ...
        //else if(strcmp(args[index], ">>")){ 
            ...
            //Do multiple steps in writing out to an output file and append contents in the existing file or if file does not exist
            //then create a new one which will involve even more steps with an if-statement to check if the files exits
            
            strcpy(appendFileOutputName, args[index+1]);
            

            //fileDes1 == '>>'
            int pidAppendOut = fork();
            if(pidAppendOut >= 0){
                if(pidAppendOut == 0){ //Child
                    //appendOutFile -> appending to output file
                    int appendOutFile = open(appendFileOutputName, O_WRONLY|O_CREAT|O_APPEND, S_IRWXU|S_IRWXG|S_IRWXO);
                    //Replace stdout with appendOutFile
                    close(1);
                    dup2(appendOutFile, 1);
                    close(appendOutFile);
                    //Attempt to execute command; print a message and exit child process if error encounter
                    if(execvp(args[0], args) < 0){
                        perror("Could not execute command!");
                        exit(-1); //Does not exit shell, only exits child process
                    }
                }
                else{ //Parent
        ...
        //else if(strcmp(args[index], ">")){
            ...
            //Do multiple steps in writing out to an output file and overwriting the contents in the existing file or if file 
            //does not exist then create a new one which will involve even more steps with an if-statement to check if the files exits. 
        ...
        //else if(strcmp(args[index], "<")) {
            ...
            printf("%d\n", fd);
            dup2(fd, 0); //All input is now read from the file ‘input.txt’
            int c;
            while((c = getchar()) != 'EOF'){
                printf("%c\n", c);
            }
    ...
    return 0; //if function fails return -1, otherwise return 0 
} 
...
int backgroundExecution(char *input){
    ...
    printf("INDEX %d: %s", index, backgroundCommand[index]);
    backgroundCommand[index] = NULL;
    //backgroundCommand[index-1] = '\0';
